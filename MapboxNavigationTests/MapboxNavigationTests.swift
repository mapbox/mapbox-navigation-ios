...
class MapboxNavigationTests: XCTestCase {
    ...
    var didFinishLoadingStyle: ((NavigationMapView) -> Void)?
    
    var didFinishLoadingMap: ((NavigationMapView) -> Void)?
    ...
    override func setUpWithError() throws {
        ...
        navigationMapView.mapView.mapboxMap.onEvery(.styleLoaded) { [weak self] _ in
            guard let self = self,
                  let navigationMapView = self.navigationMapView else {
                return
            }
            
            self.didFinishLoadingStyle?(navigationMapView)
        }
        
        navigationMapView.mapView.mapboxMap.onEvery(.mapLoaded) { [weak self] _ in
            guard let self = self,
                  let navigationMapView = self.navigationMapView else {
                return
            }
            
            self.didFinishLoadingMap?(navigationMapView)
        }
    ...
    func testHighlightBuildings() {
        didFinishLoadingStyle = { navigationMapView in
        ...
        didFinishLoadingMap = { [weak self] navigationMapView in
            ...
            self.wait(for: [featureQueryExpectation], timeout: 5.0)
            ...
            let buildingExtrusionLayer = try? navigationMapView.mapView.mapboxMap.style.layer(withId: buildingExtrusionLayerIdentifier)
    ...
    func testPuck3DLayerPosition() {
        ...
        didFinishLoadingMap = { [weak self] navigationMapView in
            ...
            guard let route: Route = try? decoder.decode(Route.self, from: self.JSONFromFile(name: "route")) else {
    ...
    func testUserLocationStyle() {
        ...
        didFinishLoadingMap = { [weak self] navigationMapView in
            ...
            self.wait()
            self.assertImageSnapshot(matching: UIImageView(image: navigationMapView.snapshot()), as: .image(precision: 0.95))
