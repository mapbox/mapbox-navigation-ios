...
@nonobjc extension FBSnapshotTestCase {
    ...
    func verify(_ view: UIView, overallTolerance: CGFloat = 0.05) {
        FBSnapshotVerifyView(view, suffixes: suffix, perPixelTolerance: overallTolerance)
    }
    
    func verify(_ layer: CALayer, overallTolerance: CGFloat = 0.05) {
        FBSnapshotVerifyLayer(layer, suffixes: suffix, perPixelTolerance: overallTolerance)
    }
}
