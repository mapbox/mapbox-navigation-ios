...
open class RouteProgress: NSObject {
    ...
    @objc public var legIndex: Int {
        didSet {
            guard legIndex >= 0 && legIndex < route.legs.endIndex else {
                print("legIndex - \(legIndex) out of range.")
                return
            }
...
open class RouteLegProgress: NSObject {
    ...
    @objc public var stepIndex: Int {
        didSet {
            guard stepIndex >= 0 && stepIndex < leg.steps.endIndex else {
                print("stepIndex - \(stepIndex) out of range.")
                return
            }
