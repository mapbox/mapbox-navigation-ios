...
open class RouteVoiceController: NSObject, AVSpeechSynthesizerDelegate, AVAudioPlayerDelegate {
     ...
     */
//    public var rerouteSoundPlayer: AVAudioPlayer = try! AVAudioPlayer(data: NSDataAsset(name: "reroute-sound", bundle: .mapboxCoreNavigation)!.data, fileTypeHint: AVFileTypeMPEGLayer3)
...
//        rerouteSoundPlayer.play()
    ...
    public func alertLevelDidChange(notification: NSNotification) {
    ...
    public func shouldSpeak(for notification: NSNotification) -> Bool {
//        guard isEnabled, volume > 0, !NavigationSettings.shared.muted else { return false }
        ...
        fallbackText = createSpeechString(notification: notification, markUpWithSSML: false)
    ...
    public func createSpeechString(notification: NSNotification, markUpWithSSML: Bool) -> String {
    ...
    public func speak(_ text: String, error: String? = nil) {
        // Note why it failed
        if let error = error {
            print(error)
        }
