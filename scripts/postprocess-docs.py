...
symbols = {
    ".".join(path.split('.')[0].split('/')[1:]):
    '{base}/{path}'.format(base=base_url, path=path)
    for path in filter(
        lambda p: (p.startswith('Classes') or p.startswith('Structs') or p.
                   startswith('Enums')) and p.endswith('.html'),
        index_data,
    )
}
