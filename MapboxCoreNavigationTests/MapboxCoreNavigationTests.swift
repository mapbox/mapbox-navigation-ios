...
let response = Fixture.JSONFromFileNamed(name: "routeWithVoiceInstructions")
...
let route = Route(json: jsonRoute, waypoints: [waypoint1, waypoint2], routeOptions: options)
...
class MapboxCoreNavigationTests: XCTestCase {
    ...
    func testVoiceAnnouncement() {
        ...
        let location = CLLocation(coordinate: CLLocationCoordinate2D(latitude: 37.79514389639814, longitude: -122.41318166255951), altitude: 1, horizontalAccuracy: 1, verticalAccuracy: 1, course: 0, speed: 10, timestamp: Date())
        ...
        self.expectation(forNotification: RouteControllerDidPassSpokenInstructionPoint.rawValue, object: navigation) { (notification) -> Bool in
            ...
            return routeProgress.currentLegProgress.stepIndex == 0 && routeProgress.currentLegProgress.currentStepProgress.voiceInstructionIndex == 0
        }
    ...
    func testShouldReroute() {
        ...
        let secondLocation = CLLocation(coordinate: CLLocationCoordinate2D(latitude: 38, longitude: -124), altitude: 0, horizontalAccuracy: 0, verticalAccuracy: 0, course: 0, speed: 0, timestamp: Date(timeIntervalSinceNow: 5))
