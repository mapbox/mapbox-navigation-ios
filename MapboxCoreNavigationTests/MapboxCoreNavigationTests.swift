...
class MapboxCoreNavigationTests: XCTestCase {
    ...
    func testShouldReroute() {
        ...
        self.expectation(forNotification: NSNotification.Name(rawValue: RouteControllerWillReroute.rawValue), object: navigation) { (notification) -> Bool in
            XCTAssertEqual(notification.userInfo?.count, 1)
    ...
    func testArrive() {
        ...
        self.expectation(forNotification: NSNotification.Name(rawValue: RouteControllerProgressDidChange.rawValue), object: navigation) { (notification) -> Bool in
            let routeProgress = notification.userInfo![RouteControllerDidPassSpokenInstructionPointRouteProgressKey] as? RouteProgress
