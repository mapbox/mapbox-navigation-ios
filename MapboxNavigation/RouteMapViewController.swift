...
class RouteMapViewController: UIViewController {
    ...
    func mapView(_ mapView: MGLMapView, viewFor annotation: MGLAnnotation) -> MGLAnnotationView? {
        if let debugAnnotation = annotation as? DebugAnnotation {
...
extension RouteMapViewController {
    func addDebugAnnotation(location: CLLocation) {
        // Only keep 100 annotations on the map
        if let annotations = mapView.annotations, annotations.count > 100 {
            mapView.removeAnnotations(Array(annotations.suffix(annotations.count - 100)))
        }
        let annotation = DebugAnnotation()
        annotation.coordinate = location.coordinate
        annotation.title = "Actual Location"
