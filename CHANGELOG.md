...
* Added the `NavigationMapView.puckType` property to customize how the user’s current location is displayed on the map. Set this property to `UserLocationStyle.puck2D` or `UserLocationStyle.puck3D` to use the [locationlayer](https://docs.mapbox.com/ios/maps/api/10.0.0-beta.20/Enums/LayerType.html#/s:10MapboxMaps9LayerTypeO17locationIndicatoryA2CmF) powered by the Mapbox Maps SDK instead of the default view-backed implementation specified by the `NavigationMapView.userCourseView` property. ([#2968](https://github.com/mapbox/mapbox-navigation-ios/pull/2968))
* Updated the `UserPuckCourseView` and `UserHaloCourseView` to open access, so that the two user course views could be customized and subclassable. ([#2968](https://github.com/mapbox/mapbox-navigation-ios/pull/2968))
