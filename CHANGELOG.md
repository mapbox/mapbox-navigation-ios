...
* Added the ability to allow developers choose between `NavigationMapView.userCourseView` and map SDK location layer during turn-by-turn navigation. By providing corresponding configuration for `.puck2D` or `.puck3D`, developers could use the location puck from `MapboxMaps` during turn-by-turn navigation. ([#2968](https://github.com/mapbox/mapbox-navigation-ios/pull/2968))
