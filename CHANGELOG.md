...
### Spoken instructions
* Introduced `SpeechSynthesizing` for implementing custom Speech Synthesizing engines, as well as built-in implementations `SystemSpeechSynthesizer` and `MapboxSpeechSynthesizer` for system SpeechSynthesizer and Mapbox Speech engine respectively. This splits `RouteVoiceController` functionality into separate parts. `MapboxVoiceController` is replaced with alternative `SpeechSynthesizersController` which allows multiple Synthesizers with a fallback mechanism. ([#2268(https://github.com/mapbox/mapbox-navigation-ios/pull/2348)])
