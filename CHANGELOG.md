...
* Removed `NavigationMapView.labelCurrentRoadFeature(at:router:wayNameView:roadNameFromStatus:)`. Instead, updating the road label through the `WayNameView.label.updateRoad(roadName:representation:)` and `WayNameView.label.updateStyle(styleURI:)`. ([#3723](https://github.com/mapbox/mapbox-navigation-ios/pull/3723))
