...
* Fixed an issue where the `UserHaloCourseView` will be shown under reduced location accuracy mode, even when `NavigationMapview.reducedAccuracyActivatedMode` is `false`. Right now `UserHaloCourseView` will be applied in only one case: when `NavigationMapView.reducedAccuracyActivatedMode` defaults to `true`, and the `Precise Location` property in the settings of current application is disabled by user. ([#4285](https://github.com/mapbox/mapbox-navigation-ios/pull/4285))
