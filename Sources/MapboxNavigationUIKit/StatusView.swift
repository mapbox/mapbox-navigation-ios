...
//        — Lowest Priority —
    ...
    func tap(_ sender: UITapGestureRecognizer) {
        ...
        if sender.state == .ended {
            let incrementer: Double = if UIApplication.shared.userInterfaceLayoutDirection == .rightToLeft {
                location.x < bounds.midX ? 0.1 : -0.1
            } else {
                location.x > bounds.midX ? 0.1 : -0.1
            }
    ...
    func manageStatuses(status: Status? = nil) {
        if statuses.isEmpty {
            ...
            // if we hide a Status and there are Statuses left in the statuses array, show the Status with highest
            // priority
    ...
    func showSimulationStatus(speed: Int) {
        ...
        let title = String.localizedStringWithFormat(
            format,
            NumberFormatter.localizedString(from: speed as NSNumber, number: .decimal)
        )
        let simulationStatus = Status(
            identifier: "USER_IN_SIMULATION_MODE",
            title: title,
            duration: .infinity,
            interactive: true,
            priority: 2
