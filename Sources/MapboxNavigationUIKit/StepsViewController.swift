...
public class StepsViewController: UIViewController, NavigationComponent {
    ...
    func rebuildDataSourceIfNecessary() -> Bool {
        ...
        if !routeProgress.isFinalLeg {
            routeProgress.navigationRoutes.mainRoute.route.legs.dropFirst(routeProgress.legIndex + 1).forEach {
                var steps = $0.steps
                // Don't include the last step, it includes nothing
                _ = steps.popLast()
    ...
    @IBAction
    func tappedDismiss(_ sender: Any) {
...
extension StepsViewController: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Since as we progress, steps are removed from the list, we need to map the row the user tapped to the actual
        // step on the leg.
        ...
        guard routeProgress.navigationRoutes.mainRoute.route.containsStep(at: legIndex, stepIndex: stepIndex)
        else { return }
...
extension StepsViewController: UITableViewDataSource {
    ...
    func titleForHeader(in section: Int) -> String? {
        ...
        let leg = routeProgress.navigationRoutes.mainRoute.route.legs[section]
