...
open class NextBannerView: UIView, NavigationComponent {
    ...
    public func onRouteProgressUpdated(_ progress: RouteProgress) {
        guard let instruction = progress.currentLegProgress.currentStepProgress.currentVisualInstruction,
              shouldShowNextBanner(for: progress)
        else {
            hide()
            return
        }
    ...
    func shouldShowNextBanner(for routeProgress: RouteProgress) -> Bool {
        let durationForNext = 15 *
            1.2 // RouteControllerHighAlertInterval * RouteControllerLinkedInstructionBufferMultiplier
        ...
        guard let upcomingStep = routeProgress.currentLegProgress.upcomingStep,
              ...
              let _ = upcomingStep.instructionsDisplayedAlongStep?.last
        else {
            return false
        }
