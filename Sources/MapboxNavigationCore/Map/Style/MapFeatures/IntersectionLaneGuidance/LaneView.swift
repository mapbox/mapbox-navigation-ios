...
extension LaneIndication {
    ...
    func dominantSide(maneuverDirection: ManeuverDirection?, drivingSide: DrivingSide) -> DrivingSide {
        ...
        if let maneuverDirection, hasLeftwardIndication, hasRightwardIndication {
...
extension LanesStyleKit {
     ...
     */
    enum Method {
        case symmetricOff((CGContext, CGRect, ResizingBehavior, UIColor, CGSize) -> Void)
        case symmetricOn((CGContext, CGRect, ResizingBehavior, UIColor, CGSize) -> Void)
        case asymmetricOff((CGContext, CGRect, ResizingBehavior, UIColor, CGSize, Bool) -> Void)
        case asymmetricMixed((CGContext, CGRect, ResizingBehavior, UIColor, UIColor, CGSize, Bool) -> Void)
        case asymmetricOn((CGContext, CGRect, ResizingBehavior, UIColor, CGSize, Bool) -> Void)
    }
    ...
    static func styleKitMethod(
        ...
        if laneIndications.count > 3 {
            ...
            if let favoredIndication, !laneIndications.contains(favoredIndication) {
        ...
        guard let method = styleKitMethod(
            turnClassifications: turnClassifications,
            favoredTurnClassification: favoredTurnClassification
        ) ?? favoredTurnClassification.flatMap(
            { styleKitMethod(turnClassifications: [$0], favoredTurnClassification: $0) }
...
class LaneView: UIView {
    ...
    /**
     ...
     is used instead.
     */
    @objc dynamic var primaryColor: UIColor = .defaultLaneArrowPrimary {
        didSet {
            setNeedsDisplay()
        }
    }
     ...
     */
    @objc dynamic var secondaryColor: UIColor = .defaultLaneArrowSecondary {
        didSet {
            setNeedsDisplay()
        }
    }
     ...
     */
    @objc dynamic var primaryColorHighlighted: UIColor = .defaultLaneArrowPrimaryHighlighted {
        didSet {
            setNeedsDisplay()
        }
    }
     ...
     */
    @objc dynamic var secondaryColorHighlighted: UIColor = .defaultLaneArrowSecondaryHighlighted {
        didSet {
            setNeedsDisplay()
        }
    }
     ...
     */
    var showHighlightedColors: Bool = false {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    static let defaultFrame: CGRect = .init(origin: .zero, size: .init(width: 30, height: 30))
    ...
    convenience init(
        ...
        self.maneuverDirection = direction ?? ManeuverDirection(rawValue: indications.description)
    ...
    override func draw(_ rect: CGRect) {
        ...
        LaneRenderer(
            ...
            primaryColorHighlighted: primaryColorHighlighted,
            secondaryColorHighlighted: secondaryColorHighlighted,
            showHighlightedColors: showHighlightedColors
        ).draw(in: UIGraphicsGetCurrentContext()!, for: traitCollection)
    }
}

extension UIColor {
    fileprivate class var defaultLaneArrowPrimary: UIColor { #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1) }
    fileprivate class var defaultLaneArrowSecondary: UIColor { #colorLiteral(red: 0.6196078431, green: 0.6196078431, blue: 0.6196078431, alpha: 1) }

    fileprivate class var defaultLaneArrowPrimaryHighlighted: UIColor { #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1) }
    fileprivate class var defaultLaneArrowSecondaryHighlighted: UIColor { UIColor(white: 0.7, alpha: 1.0) }
}
