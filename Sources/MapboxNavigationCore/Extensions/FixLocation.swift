...
extension FixLocation {
    convenience init(_ location: CLLocation, isMock: Bool = false) {
        let bearingAccuracy = location.courseAccuracy >= 0 ? location.courseAccuracy as NSNumber : nil

        var provider: String?
#if compiler(>=5.5)
        if #available(iOS 15.0, *) {
            if let sourceInformation = location.sourceInformation {
                // in some scenarios we store this information to history files, so to save space there, we use "short"
                // names and 1/0 instead of true/false
