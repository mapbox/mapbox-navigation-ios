...
public final class RouteParserSpy: RouteParser  {
    ...
    @_implementationOnly
    public override class func parseDirectionsResponse(forResponseDataRef responseDataRef: DataRef,
                                                request: String,
                                                routeOrigin: RouterOrigin) -> Expected<NSArray, NSString> {
        if returnedRoutes == nil && returnedError == nil {
            return RouteParser.parseDirectionsResponse(forResponseDataRef: responseDataRef, request: request, routeOrigin: routeOrigin)
        }
        return returnedRoutes != nil ? Expected(value: (returnedRoutes ?? []) as NSArray) :
        Expected(error: (returnedError ?? "") as NSString)
    }
