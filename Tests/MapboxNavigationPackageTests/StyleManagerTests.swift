...
class StyleManagerTests: TestCase {
    ...
    func testTimeIntervalsUntilTimeOfDayChanges() {
        ...
        XCTAssert(
            beforeSunriseAfterMidnight
                .intervalUntilTimeOfDayChanges(sunrise: sunrise, sunset: sunset) == (6 * 3600)
        )
        XCTAssert(
            afterSunriseBeforeSunset
                .intervalUntilTimeOfDayChanges(sunrise: sunrise, sunset: sunset) == (7 * 3600)
        )
        XCTAssert(
            afterSunsetBeforeMidnight
                .intervalUntilTimeOfDayChanges(sunrise: sunrise, sunset: sunset) == (10 * 3600)
        )
    }
    ...
    func testDidChangeStyleNotification() {
        ...
        let dayExpectation = expectation(
            forNotification: .styleManagerDidApplyStyle,
            object: styleManager
        ...
        let nightExpectation = expectation(
            forNotification: .styleManagerDidApplyStyle,
            object: styleManager
        ) { notification -> Bool in
            let userInfo = notification.userInfo
