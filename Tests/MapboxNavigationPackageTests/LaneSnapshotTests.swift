...
class LaneSnapshotTests: TestCase {
    override func setUp() {
        ...
        LaneView.appearance().primaryColor = LaneView.appearance(whenContainedInInstancesOf: [LanesView.self])
            .primaryColor
        LaneView.appearance().secondaryColor = LaneView.appearance(whenContainedInInstancesOf: [LanesView.self])
            .secondaryColor
        LaneView.appearance().primaryColorHighlighted = LaneView
            .appearance(whenContainedInInstancesOf: [LanesView.self]).primaryColorHighlighted
        LaneView.appearance().secondaryColorHighlighted = LaneView
            .appearance(whenContainedInInstancesOf: [LanesView.self]).secondaryColorHighlighted
        LanesView.appearance().backgroundColor = #colorLiteral(red: 0.968627451, green: 0.968627451, blue: 0.968627451, alpha: 1)
    }

    func testAllLanesSize30() {
        verifyAllLanes(size: CGSize(size: 30))
    }

    func testAllLanesSize90() {
    ...
    func verifyAllLanes(size: CGSize) {
        ...
        func addLanes(lanes: [TestableLane], stackView: UIStackView) {
            ...
            for lane in lanes {
                ...
                let laneView = LaneView(
                    ...
                    isUsable: true,
                    direction: lane.maneuverDirection
                )
        ...
        assertImageSnapshot(matching: view, as: .image(precision: 0.99), named: "verifyAllLanes\(size)")
    }
}
...
struct TestableLane {
    ...
    static func testableLanes(drivingSide: DrivingSide) -> [TestableLane] {
        ...
        return namedIndications.map { TestableLane(
            description: $0.0,
            indications: $0.1,
            drivingSide: drivingSide,
            maneuverDirection: $0.2
        ) }
    }
}
