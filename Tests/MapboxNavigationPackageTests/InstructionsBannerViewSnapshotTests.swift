...
class InstructionsBannerViewSnapshotTests: InstructionBannerTest {
    ...
    override func setUp() {
        ...
        let us101Representation = VisualInstruction.Component
            .ImageRepresentation(imageBaseURL: ShieldImage.us101.baseURL)
    ...
    func testSinglelinePrimary() {
        ...
        view.update(for: makeVisualInstruction(
            .turn,
            .right,
            primaryInstruction: instructions,
    ...
    func testMultilinePrimary() {
        ...
        let instructions: [VisualInstruction.Component] = [
            .image(
                image: .init(imageBaseURL: ShieldImage.i280.baseURL),
                alternativeText: .init(text: "I 280", abbreviation: nil, abbreviationPriority: 0)
            ),
    ...
    func testSinglelinePrimaryAndSecondary() {
        ...
        let secondary = [VisualInstruction.Component.text(text: .init(
            text: "US 45 / Chicago",
            abbreviation: nil,
    ...
    func testSinglelinePrimaryAndSecondaryWithShield() {
        ...
        let i280Shield = VisualInstruction.Component.ShieldRepresentation(
            baseURL: spriteRepository.baseURL,
            name: "us-interstate",
            textColor: "white",
            text: "280"
        )
        let i280Representation = VisualInstruction.Component.ImageRepresentation(
            imageBaseURL: ShieldImage.i280.baseURL,
            shield: i280Shield
        )

        let primary: [VisualInstruction.Component] = [
            .image(
                image: i280Representation,
                alternativeText: .init(text: "I 280", abbreviation: nil, abbreviationPriority: 0)
            ),
            .text(text: .init(text: "South", abbreviation: nil, abbreviationPriority: 0)),
        ]
    ...
    func testAbbreviateInstructionsIncludingDelimiter() {
        ...
        let primary: [VisualInstruction.Component] = [
            .image(
                image: .init(imageBaseURL: ShieldImage.i280.baseURL),
                alternativeText: .init(text: "I 280", abbreviation: nil, abbreviationPriority: nil)
            ),
    ...
    func testAdjacentShields() {
        ...
        let primary: [VisualInstruction.Component] = [
            ...
            .delimiter(text: .init(text: "/", abbreviation: nil, abbreviationPriority: nil)),
            .image(
                image: .init(imageBaseURL: ShieldImage.us101.baseURL),
                alternativeText: .init(text: "US-101", abbreviation: nil, abbreviationPriority: nil)
            ),
        ]
    ...
    func testInstructionsAndNextInstructions() {
        ...
        let nextBannerViewFrame = CGRect(
            x: 0,
            y: instructionsBannerView.frame.maxY,
            width: instructionsBannerView.bounds.width,
            height: 44
        )
        ...
        view.frame = CGRect(
            origin: .zero,
            size: CGSize(width: nextBannerViewFrame.width, height: nextBannerViewFrame.maxY)
        )
        ...
        let primary = [
            VisualInstruction.Component.image(
                image: .init(imageBaseURL: ShieldImage.i280.baseURL),
                alternativeText: .init(text: "I 280", abbreviation: nil, abbreviationPriority: 0)
            ),
        ]
        ...
        instructionsBannerView.update(for: makeVisualInstruction(
            .turn,
            .right,
            primaryInstruction: primary,
            secondaryInstruction: secondary
        ...
        let primaryThenInstruction = VisualInstruction(
            text: nil,
            maneuverType: .none,
            maneuverDirection: .none,
            components: primaryThen
        )
    ...
    func testGenericShields() {
        ...
        let primary: [VisualInstruction.Component] = [
            .image(
                image: .init(imageBaseURL: nil),
                alternativeText: .init(text: "ANK 1", abbreviation: nil, abbreviationPriority: nil)
            ),
    ...
    func testGenericShieldAndExitViewWithCustomDayStyle() {
        ...
        let visualInstructionBanner = makeVisualInstruction(
            .takeOffRamp,
            .right,
        ...
        assertImageSnapshot(
            matching: instructionsBannerView,
...
class InstructionBannerTest: TestCase {
    ...
    func makeVisualInstruction(
        ...
        return Fixture.makeVisualInstruction(
    ...
    func cacheSprite(for styleURI: StyleURI = .light) {
        let shieldImage: ShieldImage = (styleURI == .dark) ? .shieldNight : .shieldDay
        guard let styleID = spriteRepository.styleID(for: styleURI),
              ...
              let spriteData = shieldImage.image.pngData()
        else {
            XCTFail("Failed to form request URL.")
            return
        }

        let spriteResponse = URLResponse(
            url: spriteRequestURL,
            mimeType: nil,
            expectedContentLength: spriteData.count,
            textEncodingName: nil
        )
        spriteRepository.requestCache.store(
            CachedURLResponse(response: spriteResponse, data: spriteData),
            for: spriteRequestURL
        )

        let infoData = Fixture.JSONFromFileNamed(name: "sprite-info")
        let infoResponse = URLResponse(
            url: infoRequestURL,
            mimeType: nil,
            expectedContentLength: infoData.count,
            textEncodingName: nil
        )
        spriteRepository.requestCache.store(
            CachedURLResponse(response: infoResponse, data: infoData),
            for: infoRequestURL
        )
    }
    ...
    func cacheLegacyIcon(
        with representation: VisualInstruction.Component.ImageRepresentation,
        shieldImage: ShieldImage
    ) {
        guard let legacyURL = representation.imageURL(scale: VisualInstruction.Component.scale, format: .png),
              let data = shieldImage.image.pngData()
        else {
