...
class RouteTests: TestCase {
    var options: NavigationMatchOptions!
    ...
    override func setUp() {
        ...
        options = NavigationMatchOptions(coordinates: [
    ...
    func testMatchPolylineAroundManeuver() {
        let mapMatchingResponse = Fixture.mapMatchingResponse(from: "route-doubling-back", options: options)
        let routes = mapMatchingResponse.matches
        ...
        for stepIndex in traversals {
            ...
            assert(maneuverPolyline: maneuverPolyline,
                   stepPolyline: stepPolyline,
                   precedingStepPolyline: precedingStepPolyline,
                   stepIndex: stepIndex,
                   followingStep: followingStep)
        }

        XCTAssertNoThrow(route.polylineAroundManeuver(legIndex: 0, stepIndex: 100, distance: 50))
    }

    func testRoutePolylineAroundManeuver() {
        let routes = Fixture.routesFromMatches(at: "route-doubling-back", options: options)
        let route = routes!.first!
        let leg = route.legs.first!
    ...
    private func assert(maneuverPolyline: LineString,
                        stepPolyline: LineString,
                        precedingStepPolyline: LineString,
                        stepIndex: Int,
