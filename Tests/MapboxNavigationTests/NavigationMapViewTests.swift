...
class NavigationMapViewTests: TestCase {
    ...
    // If fractionTraveled is higher than first restricted region, the next region will become restricted as well.
    func testGenerateRouteLineRestrictedGradientBug() {
        let coordinates: [CLLocationCoordinate2D] = [
            CLLocationCoordinate2D(latitude: 1, longitude: 0),
            CLLocationCoordinate2D(latitude: 2, longitude: 0),
            CLLocationCoordinate2D(latitude: 3, longitude: 0),
    ...
    func testWaypointLayerPosition() {
        let navigationMapView = NavigationMapView(frame: UIScreen.main.bounds)
        ...
        wait(for: [mapLoadingErrorExpectation], timeout: 5.0)

        let coordinates: [CLLocationCoordinate2D] = [
            CLLocationCoordinate2D(latitude: 37.81286886222617, longitude: -122.2784401698491),
            CLLocationCoordinate2D(latitude: 37.81286886222617, longitude: -122.2784401698491)
        ]

        let customFixture = Fixture.route(waypoints: coordinates)
        
        navigationMapView.show([customFixture.route])
        navigationMapView.showWaypoints(on: customFixture.route)
        
        // Style doesn't contain any layers besides the waypoint layer, main route layer, and the main route casing layer.
        // If layer position wasn't provided, waypoint later should be placed
        // above the main route layer.
        // Since the waypoint layer identifier is generated by `showWaypoints(on:layerPosition:legIndex:)`,
        // we must verify by layer type.
        XCTAssertEqual(navigationMapView.mapView.mapboxMap.style.allLayerIdentifiers[safe: 0]?.type, MapboxMaps.LayerType.symbol, "Waypoint symbol layer is the only symbol layer present in the style, so layer types should be equal.")
        
        XCTAssertEqual(navigationMapView.mapView.mapboxMap.style.allLayerIdentifiers[safe: 1]?.type, MapboxMaps.LayerType.line, "Route line layer type should be equal.")
        ...
        navigationMapView.showWaypoints(on: customFixture.route, layerPosition: .above(layerIdentifier))
        
        // If a layer position is provided to be placed above a specific later,
        // waypoint symbol layer should be placed above that specific later.
        
        XCTAssertEqual(navigationMapView.mapView.mapboxMap.style.allLayerIdentifiers[safe: 1]?.id,
                       layerIdentifier,
                       "Custom line layer identifiers should be equal.")
        
        XCTAssertEqual(navigationMapView.mapView.mapboxMap.style.allLayerIdentifiers[safe: 0]?.type,
                       MapboxMaps.LayerType.symbol,
                       "Route line casing layer identifiers should be equal.")
    }
}
